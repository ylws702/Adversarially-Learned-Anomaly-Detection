...
def _get_dataset(scale):
    ...
    labels[labels != 2] = 0
    labels[labels == 2] = 1
    df = df.iloc[:, 1:].apply(np.fft.fft).apply(np.abs)
    df['labels'] = labels
    ...
    x_train, y_train = _to_xy(df_train, target='labels')
    x_valid, y_valid = _to_xy(df_valid, target='labels')
    x_test, y_test = _to_xy(df_test, target='labels')
